...
HalStatus AesHal::Decrypt(const uint8_t* in, uint8_t* out, size_t length, const HalOptions& options) {
    CRYPTO->DATA_IN = static_cast<uint32_t>(in);
    uint32_t _s = 0u;
    _s |= CRYPTO->STAT;
    (void)_s;
...
HalStatus ShaHal::Final(uint8_t* digest, size_t length, const HalOptions& options) {
...
HalStatus ShaHal::Digest(const uint8_t* data, size_t length, uint8_t* out, uint32_t timeout_ms) {
    CRYPTO->DATA_IN = static_cast<uint32_t>(data);
    uint32_t _s = 0u;
    _s |= CRYPTO->STAT;
    (void)_s;
